...
void experiment(std::string input, std::string output, int verbose){
    auto A_desc = json::parse(path(input)/"A"/"binsparse.json"); 
    auto x_desc = json::parse(path(input)/"x"/"binsparse.json"); 
...
void experiment_spmv_csr(std::string input, std::string output, int verbose){
    ...
    auto x_val = npy_load_vector<T>(path(input)/"x"/"values.npy");
    auto A_ptr = npy_load_vector<I>(path(input)/"A"/"pointers_to_1.npy");
    auto A_ind = npy_load_vector<I>(path(input)/"A"/"indices_1.npy");
    auto A_val = npy_load_vector<I>(path(input)/"A"/"values.npy");
    ...
    std::ofstream results_file(path(output)/"y"/"y.json");
    results_file << results;
    results_file.close();

    npy_store_vector<T>(path(output)/"y"/"values.npy", y_val);

    json measurements;
    measurements["time"] = 0;
    measurements["memory"] = 0;
    std::ofstream measurements_file(path(output)/"measurements.json");
