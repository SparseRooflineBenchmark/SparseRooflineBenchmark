...
void experiment_spmspv_csr(benchmark_params_t params);
...
int main(int argc, char **argv){
    ...
    if (A_desc["data_types"]["pointers_to_1"] == "int32" &&
        ...
        A_desc["data_types"]["values"] == "float64") {
            experiment_spmspv_csr<double, int64_t>(params);
    } else {
...
void experiment_spmspv_csr(benchmark_params_t params){
    ...
    //perform an spmspv of the matrix in c++
    ...
    fs::create_directory(fs::path(params.output)/"y.bspnpy");
    json y_desc;
    y_desc["version"] = 0.5;
    y_desc["format"] = "DVEC";
    y_desc["shape"] = {n};
